...
export function Dialog({ open, onOpenChange, children }: { open: boolean, onOpenChange: (open: boolean) => void, children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div className="relative z-50 w-full max-w-lg shadow-lg duration-200 animate-in fade-in-90 zoom-in-95">
        {children}
      </div>
    </div>
  );
...
export function DialogContent({ children, className, onClose }: { children: React.ReactNode, className?: string, onClose?: () => void }) {
  return (
    <div className={cn("relative rounded-lg bg-white p-6 shadow-xl", className)}>
      {onClose && (
        <button
          onClick={onClose}
